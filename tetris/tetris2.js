...
// Array to store the queue of pieces for pseudo random algorithim
let pieceQueue = [];
...
function moveDown() {
    ...
    if (collide(board, player)) {
        ...
        score += Math.round(2*scoreMultiplier); // Reward for placing a block
        drawScore();    // Show the new score
    }
    dropCounter = 0;
}
...
function fetchLeaderboard() {
    ...
    fetch('https://zoomingguy.duckdns.org:7322/leaderboard')
...
function enterScore() {
    ...
    fetch('https://zoomingguy.duckdns.org:7322/updateLeaderboard', {
        method: 'POST',
        headers: {
    ...
    .then(response => {
        if (response.ok) {
            ...
            return fetch('https://zoomingguy.duckdns.org:7322/leaderboard');
        } else {
...
function drawLeaderboardEntries(leaderboardData) {
    ...
    context.font = 'bold 25px "Andale Mono", monospace'; // Using Andale Mono as a monospaced font
    ...
    leaderboardDisplay.innerHTML = ''; // Clear previous entries
    const leaderboardEntries = leaderboardData.map((entry, index) => {
        const num = (index + 1).toString().padStart(2, '\u00A0'); // \u00A0 is a non breaking space
        const name = entry.name.padEnd(14, '.'); // Name with trailing dots, adjusted for 10 characters
        const score = entry.score.toString().padStart(8, '.'); // Score with leading spaces
    ...
    leaderboardDisplay.innerHTML = leaderboardEntries.join('');
}
...
// Function to refill the piece queue
function refillPieceQueue() {
    const pieces = ['I', 'L', 'J', 'O', 'T', 'S', 'Z'];
    pieceQueue = [];
    for (const piece of pieces) {
        pieceQueue = pieceQueue.concat(Array(2).fill(piece));
    }
    pieceQueue = shuffleArray(pieceQueue);
}
